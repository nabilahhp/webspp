	...
	<div class="input-group">
		...
		<select name="id_kategori_staff" class="form-control">
			<?php foreach ($kategori_staff as $kategori) {
				if ($kategori->nama_kategori_staff == 'Administrator') continue; // skip administrator
			?>
				<option value="<?php echo $kategori->id_kategori_staff ?>">
					<?php echo $kategori->nama_kategori_staff ?>
				</option>
			<?php } ?>
		</select>
		<span class="input-group-append">
